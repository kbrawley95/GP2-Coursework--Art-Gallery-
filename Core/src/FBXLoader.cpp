...
void processNode(FbxNode *node, shared_ptr<GameObject> parent)
{
	...
	currentGameObject->AddComponent<Mesh>();
	...
	currentGameObject->transform.position = Vector3(translation[0], translation[1], translation[2]);
	currentGameObject->transform.position = Vector3(rotation[0], rotation[1], rotation[2]);
	currentGameObject->transform.position = Vector3(scaling[0], scaling[1], scaling[2]);
...
void processMesh(FbxMesh * mesh, shared_ptr<GameObject> gameObject)
{
	...
	shared_ptr<Mesh> m = gameObject->GetComponent<Mesh>();
	...
	for (int i = 0; i < numVerts; i++)
	{
		...
		m->vertices[i].position = vec3(currentVert[0], currentVert[1], currentVert[2]);
		m->vertices[i].colour = vec4(1.0f, 1.0f, 1.0f, 1.0f);
		m->vertices[i].normal = vec3(0.0f, 0.0f, 0.0f);
		m->vertices[i].texCoords = vec2(0.0f, 0.0f);
	}

	processMeshTextureCoords(mesh, m);
	processMeshNormals(mesh, m);
	...
	for (int iPolygon = 0; iPolygon < mesh->GetPolygonCount(); iPolygon++) {
		for (unsigned iPolygonVertex = 0; iPolygonVertex < 3; iPolygonVertex++) {
			...
			if (fbxLayerUV) {
				...
				m->vertices[fbxCornerIndex].texCoords.x = fbxUV[0];
				m->vertices[fbxCornerIndex].texCoords.y = 1.0f - fbxUV[1];
			}
		}
	}
}
...
void processMeshNormals(FbxMesh * mesh, shared_ptr<Mesh> m)
{
	for (int iPolygon = 0; iPolygon < mesh->GetPolygonCount(); iPolygon++) {
		for (unsigned iPolygonVertex = 0; iPolygonVertex < 3; iPolygonVertex++) {
			...
			fbxNormal.Normalize();
			m->vertices[fbxCornerIndex].normal.x = fbxNormal[0];
			m->vertices[fbxCornerIndex].normal.y = fbxNormal[1];
			m->vertices[fbxCornerIndex].normal.z = fbxNormal[2];
		}
	}
}